...
public class adminPage2 extends JFrame {
		...
		btn_regist.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(cnt>0) {
						JOptionPane.showMessageDialog(null, "µî·Ï ¿Ï·á", "µî·Ï ¿Ï·á", JOptionPane.INFORMATION_MESSAGE);
						adminPage ap =new adminPage();
						ap.setVisible(true);
						dispose();
	...
	class UploadActionListener implements ActionListener {
	      ...
	      UploadActionListener() {
	         ...
	         FileNameExtensionFilter filter=new FileNameExtensionFilter("png ÆÄÀÏ", "png");
	         FileNameExtensionFilter filter2=new FileNameExtensionFilter("jpg ÆÄÀÏ", "jpg");
	...
	public void uploadFile(String clothpath,String genderpath,String filename) {
	      ...
	      try{
	         ...
	         fout = new FileOutputStream("src\\clothes\\"+clothpath+"\\"+genderpath+"\\"+filename); //È®Àå¸í jpg´Â ¹Ù²Ù¸é ¾ÈµÊ
	         while(true) {
	            data = fin.read();
	            if( data == -1) break;
	            fout.write(data); 
	         ...
	         System.out.println();
	         System.out.println(selectedFile.getName() + " ÀúÀå ¿Ï·á");

	      }
	      catch(IOException e1) {
	         System.out.println("ÆÄÀÏ¿ÀÇÂ½ÇÆÐ");
	         e1.printStackTrace();
	      }
	      finally {
	         try   {
